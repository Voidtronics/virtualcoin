...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        connect(ui->customFee, &VirtualcoinAmountField::valueChanged, this, &SendCoinsDialog::coinControlUpdateLabels);
        ...
        if (model->privateKeysDisabled()) {
            ...
            ui->sendButton->setToolTip(tr("Creates a Partially Signed Virtualcoin Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
        }
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus,
        VirtualcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients())
    {
        ...
        QString amount = VirtualcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
    ...
    if (model->privateKeysDisabled()) {
        questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed Virtualcoin Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(VirtualcoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    for (const VirtualcoinUnits::Unit u : VirtualcoinUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(VirtualcoinUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
        .arg(VirtualcoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::setBalance(const interfaces::WalletBalances& balances)
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(VirtualcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(VirtualcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->wallet().getDefaultMaxTxFee()));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(VirtualcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) + "/kB");
    }
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    ui->labelSmartFee->setText(VirtualcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Virtualcoin address"));
        }
        else // Valid address
        {

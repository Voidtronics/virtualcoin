// Copyright (c) 2016-2019 The Virtualcoin Core developers
...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s virtualcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "virtualcoin-wallet is an offline tool for creating and interacting with Virtualcoin Core wallet files.\n" +
                                      "By default virtualcoin-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
                                      "To change the target wallet, use the -datadir, -wallet and -testnet/-regtest arguments.\n\n" +
                                      "Usage:\n" +
                                     "  virtualcoin-wallet [options] <command>\n\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `virtualcoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }

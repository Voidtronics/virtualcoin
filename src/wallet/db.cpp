...
//! so virtualcoin should never create different databases with the same fileid, but
...
bool BerkeleyEnvironment::Open(bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of virtualcoin may be using it.\n", strPath);
        return false;
    }
...
BerkeleyBatch::BerkeleyBatch(BerkeleyDatabase& database, const char* pszMode, bool fFlushOnCloseIn) : pdb(nullptr), activeTxn(nullptr)
{
...

    {
        ...
        if (pdb == nullptr) {
            ...
            // virtualcoin from opening the same data file through another

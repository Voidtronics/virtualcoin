// Copyright (c) 2017-2019 The Virtualcoin Core developers
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 VTC in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
        ...
        BOOST_CHECK_EQUAL(nValueRet, 2 * COIN);  // we should get 2 VTC in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
